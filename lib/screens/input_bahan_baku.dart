...
class InputBahanBakuScreen extends StatefulWidget {
  const InputBahanBakuScreen({super.key});

  @override
  State<InputBahanBakuScreen> createState() => _InputBahanBakuScreenState();
}

class _InputBahanBakuScreenState extends State<InputBahanBakuScreen> {
  ...
  Future<void> _fetchStokList() async {
    final list = await FirestoreService.getStokList(Kategori.bahanBaku);
    ...
    if (_transactionType == 'Pembelian') {
      if (_priceController.text.trim().isEmpty ||
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Harga harus diisi untuk pembelian.')),
        );
        return;
      }
      if (_nameController.text.trim().isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Nama bahan baku harus diisi.')),
        );
        return;
      }
    }
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
                  ...
                  fieldViewBuilder: (ctx, textController, focusNode, onSubmit) {
                    return TextFormField(
                      ...
                      validator: (v) => (v == null || v.isEmpty)
                          ? 'Mohon masukkan nama bahan baku.'
                          : null,
                    );
                  },
                ),
          ...
          // History — single stream from `transaksi` collection
